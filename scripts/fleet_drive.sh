...
#
#   ./scripts/fleet_drive.sh                 drive every store in storelist/stores.txt
#   ./scripts/fleet_drive.sh 658 660         drive only these stores
#   FLEET_REF=<commit> ./scripts/fleet_drive.sh    pin worktrees to a commit
#
...
#   - PROD queue on READY: appends to logs/promote_queue; with PROMOTE_AUTO=1
#     and GROCER_PROD_URL + GROCER_ADMIN_TOKEN set, runs promote_stores.sh
...
#
#   PROMOTE_AUTO=1 GROCER_PROD_URL=... GROCER_ADMIN_TOKEN=... ./scripts/fleet_drive.sh
#
...
place() {
    ...
    if [ -d "$ROOT/data/$s-atlas" ]; then
        git -C "$ROOT" add -f -- "data/$s-atlas"
        paths="data/$s-atlas"
        if [ -f "$ROOT/data/$s/store.json" ]; then
            git -C "$ROOT" add -f -- "data/$s/store.json"
...
# Append a READY store to the PROD promote queue. With PROMOTE_AUTO=1 and
# PROD secrets set, drain it immediately; otherwise the operator runs
# scripts/promote_stores.sh later.
enqueue_promote() {
    s=$1
    queue="${PROMOTE_QUEUE:-$ROOT/logs/promote_queue}"
    mkdir -p "$(dirname "$queue")"
    touch "$queue"
    if ! grep -qx "$s" "$queue" 2>/dev/null; then
        echo "$s" >> "$queue"
    fi
    if [ "${PROMOTE_AUTO:-0}" = 1 ] \
            && [ -n "${GROCER_PROD_URL:-}" ] \
            && [ -n "${GROCER_ADMIN_TOKEN:-}" ]; then
        say "promote $s — PROMOTE_AUTO draining queue"
        if (cd "$ROOT" && ./scripts/promote_stores.sh "$s") \
                >> "$LOG/$s.log" 2>&1; then
            say "PROMOTED $s — data + states on PROD"
        else
            say "PROMOTE FAIL $s — left on queue; see $LOG/$s.log"
        fi
    else
        say "QUEUED $s — run ./scripts/promote_stores.sh"
    fi
}
...
STORES_TXT="$ROOT/storelist/stores.txt"
if [ $# -gt 0 ]; then
    LIST=$(for s in "$@"; do grep "^$s " "$STORES_TXT" || echo "$s"; done)
else
    LIST=$(cat "$STORES_TXT")
fi
