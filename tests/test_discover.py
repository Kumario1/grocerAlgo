...
def checks_for(path, store):
    """preflight over a guides/ PDF, skipped when it isn't present locally."""
    if not Path(path).exists():
        pytest.skip(f"{path} is not in guides/")
...
def test_full_page_scanned_store_guide_is_valid_input():
    assert validate("guides/guide-cedar-park-265.pdf", "265")[0] is None


def test_guide_drawn_before_a_remodel_is_flagged():
    flagged = checks_for("guides/guide-austin-388.pdf", "388")
    assert flagged["guide_year"] == 2011
    assert flagged["stale_risk"] is True


def test_current_guide_carries_no_stale_flags():
    assert checks_for("guides/guide-plano-790.pdf", "790") == {
...
def test_explicit_city_never_reuses_another_citys_local_guide(
    ...
    Path("guides").mkdir()
    Path("guides/guide-austin-265.pdf").touch()
    ...
    assert found == "guides/guide-cedar-park-265.pdf"
